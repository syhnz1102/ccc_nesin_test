...
### Remain issues
```
- v 화면공유 종료시 화상통화 화면이 잠시 켜졌다가 꺼지는 현상
- v (v0.9.1 추가) 영상통화 -> 종료 -> 화면공유 -> 종료 시 영상통화 화면이 남아있는 문제
- v (v0.9.1 추가) 영상통화/화면공유 시작 후 바로 통화 종료를 하면, 학생 화면에서 자신의 영상만 나오며, 통화종료 되지 않는 현상 (blocked)
- v (v0.9.2 추가) 통화중 카메라/마이크 off 후 학생 화면에서 화면 숨김 버튼을 눌렀다가 다시 화상 화면을 켜는 경우 카메라/마이크 off가 적용되지 않는 현상
```
