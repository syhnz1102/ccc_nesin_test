...
```
# v0.9.5 (20/10/23)
- 상담사화면의 상담종료 클릭시 윈도우 닫기
- 방 생성시 roomId를 랜덤이 아닌 직접 지정하도록 변경
- 이슈 2건 수정 완료
 + (v0.9.4 추가) 방 생성 후 제3자가 해당 방에 입장 시도 후 종료시 상담중인 학생화면이 상담종료되는 현상
 + (v0.9.4 추가) 공유화면 선택 팝업의 취소버튼 클릭시 학생 화면 안닫히는 현상
